...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        # SO_READDRESS is a socket option that allows the socket to be bound to an address that is already in use.
        ...
        while True:
            try:
                ...
                '''
                math_question = client_socket.recv(8192).decode()
                steps: list[str] = []
                answer = calculate(math_question, steps)
                client_socket.send(bytes(answer.encode()))
                '''
